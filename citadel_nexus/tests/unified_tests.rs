...
use citadel_nexus::error::{NexusResult, NexusError};
use citadel_nexus::traits::{StreamStats, SecurityInfo};
...
mod error_handling_tests {
    ...
    fn test_connection_error_handling() {
        let result: NexusResult<()> = Err(NexusError::Connection("Mock connection failed".to_string()));
    ...
    fn test_not_supported_error() {
        let result: NexusResult<()> = Err(NexusError::NotSupported("WebSocket servers not supported in browsers".to_string()));
        assert!(result.is_err());
