...
async fn driver<'a, T: ReliableOrderedConnectionToTarget + 'a>(conn: &'a T, node_type: RelativeNodeType, encrypted_config_container: EncryptedConfigContainer) -> Result<HolePunchedUdpSocket, anyhow::Error> {
    ...
    match node_type {
        RelativeNodeType::Receiver => {
            ...
            DualStackUdpHolePuncher::new(RelativeNodeType::Receiver, encrypted_config_container, conn, &nat_type, &nat_syn_ack.nat_type, nat_syn_ack.internal_bind_port, 100)?.await
        ...
        RelativeNodeType::Initiator => {
            ...
            tokio::time::sleep(Duration::from_nanos(delta as _)).await;
             ...
             DualStackUdpHolePuncher::new(RelativeNodeType::Initiator, encrypted_config_container, conn, &nat_type, &nat_syn.nat_type, nat_syn.internal_bind_port, 100)?.await
