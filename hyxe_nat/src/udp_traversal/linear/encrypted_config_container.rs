...
pub struct EncryptedConfigContainer {
    ...
    generate_packet: Box<dyn Fn(u16) -> BytesMut + Send + Sync + 'static>,
    // the input into the function are the encrypted bytes
    decrypt_packet: Box<dyn Fn(&[u8]) -> Option<BytesMut> + Send + Sync + 'static>
}

impl EncryptedConfigContainer {
    /// Wraps the provided functions into a portable abstraction
    pub fn new(generate_packet: impl Fn(u16) -> BytesMut + Send + Sync + 'static, decrypt_packet: impl Fn(&[u8]) -> Option<BytesMut> + Send + Sync + 'static) -> Self {
        Self { generate_packet: Box::new(generate_packet), decrypt_packet: Box::new(decrypt_packet) }
    }

    /// Generates a packet
    pub fn generate_packet(&self, local_port: u16) -> BytesMut {
