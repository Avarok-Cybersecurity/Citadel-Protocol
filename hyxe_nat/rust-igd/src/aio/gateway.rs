...
impl Gateway {
    ...
    pub async fn get_any_address(
        ...
        let port = self
            .add_any_port(protocol, remote_peer, local_addr, lease_duration, &description)
            .await?;
    ...
    pub async fn add_any_port(
        ...
        if let Some(schema) = schema {
            ...
            gateway
                .retry_add_random_port_mapping(protocol, remote_peer, local_addr, lease_duration, &description)
                .await
        }
    }
    ...
    async fn retry_add_random_port_mapping(
        ...
        for _ in 0u8..20u8 {
            match self
                .add_random_port_mapping(protocol, remote_peer, local_addr, lease_duration, &description)
                .await
            {
    ...
    async fn add_random_port_mapping(
        ...
        let external_port = common::random_port();
        let res = self
            .add_port_mapping(protocol, remote_peer, external_port, local_addr, lease_duration, &description)
            .await;
        ...
        match res {
            ...
            Err(err) => match parsing::convert_add_random_port_mapping_error(err) {
                ...
                None => {
                    gateway
                        .add_same_port_mapping(protocol, remote_peer, local_addr, lease_duration, &description)
                        .await
                }
            },
        }
    ...
    async fn add_same_port_mapping(
        ...
        let res = self
            .add_port_mapping(protocol, remote_peer,local_addr.port(), local_addr, lease_duration, description)
            .await;
        match res {
    ...
    pub async fn remove_port(&self, protocol: PortMappingProtocol, remote_peer: Option<IpAddr>, external_port: u16) -> Result<(), RemovePortError> {
        let res = self
            .perform_request(
