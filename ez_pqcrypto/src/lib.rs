...
pub type AntiReplayAttackContainer = crate::replay_attack_container::AntiReplayAttackContainer;
    ...
    fn get_symmetric_key(
        ...
        let (chain, alice_key, bob_key) = if let Some(prev) = previous_chain {
            ...
            let chain = RecursiveChain::new(chain.as_slice(), alice_key, bob_key, false)
                .ok_or_else(|| get_generic_error("Unable to create recursive chain (A)"))?;
            ...
            let alice_key =
                ...
                .ok_or_else(|| get_generic_error("Unable to create recursive chain (B)"))?;

            let bob_key = aes_gcm_siv::aead::generic_array::GenericArray::<u8, _>::from_exact_iter(
                bob_key.as_slice().iter().cloned(),
            )
            .ok_or_else(|| get_generic_error("Unable to create recursive chain (C)"))?;
            ...
            let chain = RecursiveChain::new(chain.as_slice(), alice_key, bob_key, true)
                .ok_or_else(|| get_generic_error("Unable to create recursive chain (D)"))?;

            let alice_key =
                aes_gcm_siv::aead::generic_array::GenericArray::<u8, _>::from_exact_iter(
                    alice_key.iter().cloned(),
                )
                .ok_or_else(|| get_generic_error("Unable to create recursive chain (E)"))?;

            let bob_key = aes_gcm_siv::aead::generic_array::GenericArray::<u8, _>::from_exact_iter(
                bob_key.iter().cloned(),
            )
            .ok_or_else(|| get_generic_error("Unable to create recursive chain (F)"))?;
...
pub(crate) mod function_pointers {
    ...
    pub(crate) static BOB_FP: [fn(&[u8]) -> Result<Box<dyn PostQuantumType>, Error>;
        ALGORITHM_COUNT] = [
