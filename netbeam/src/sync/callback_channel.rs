...
pub struct CallbackChannel<T, R> {
    inner: Arc<CallbackChannelInner<T, R>>
}
...
pub struct CallbackChannelPayload<T, R> {
    id: u64,
    pub payload: T,
    _pd: PhantomData<R>
}

impl<T, R> CallbackChannelPayload<T, R> {
    pub fn new(&self, payload: R) -> CallbackChannelPayload<R, T> {
        CallbackChannelPayload { id: self.id, payload, _pd: Default::default() }
    }
...
impl<T: Send + Sync, R: Send + Sync> CallbackChannel<T, R> {
    pub fn new(buffer: usize) -> (Self, CallbackReceiver<T, R>) {
        let (to_channel, from_channel) = tokio::sync::mpsc::channel(buffer);
        (Self { inner: Arc::new(CallbackChannelInner { to_channel, map: Mutex::new(Default::default()), id: AtomicU64::new(1) }) }, CallbackReceiver { inner: from_channel })
    }
    ...
    pub async fn send(&self, payload: T) -> Result<R, CallbackError<T>> {
        ...
        self.inner.to_channel.send(CallbackChannelPayload { id, payload, _pd: Default::default() }).await.map_err(|err| CallbackError::SendError(err.0.payload))?;

        Ok(rx.await.map_err(|_| CallbackError::RecvError)?)
    }

    pub async fn send_no_callback(&self, payload: T) -> Result<(), CallbackError<T>> {
        self.inner.to_channel.send(CallbackChannelPayload { id: NO_RESPONSE, payload, _pd: Default::default() }).await.map_err(|err| CallbackError::SendError(err.0.payload))
    }

    pub async fn reply(&self, payload: CallbackChannelPayload<R, T>) -> Result<(), CallbackError<R>> {
        let sender = {
            self.inner.map.lock().remove(&payload.id).ok_or_else(|| CallbackError::InternalError("Mapping does not exist for id"))?
        };

        sender.send(payload.payload).map_err(|err| CallbackError::SendError(err))
    }
}
...
mod tests {
    ...
    async fn main() {
        setup_log();
        let (tx0, mut rx) = CallbackChannel::<u32, u64>::new(10);
        let tx1 = tx0.clone();
