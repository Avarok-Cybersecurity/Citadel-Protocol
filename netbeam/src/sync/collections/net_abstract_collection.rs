...
pub struct NetAbstractCollection<
    T: NetObject,
    K,
    C: AbstractCollection<K, NetRwLock<T, S>>,
    S: Subscribable + 'static,
> {
...
impl<T: NetObject, K, C: AbstractCollection<K, NetRwLock<T, S>>, S: Subscribable + 'static>
    NetAbstractCollection<T, K, C, S>
{
    pub fn new(conn: &S) -> NetVecLoader<'_, T, K, C, S> {
        NetVecLoader {
            inner: Box::pin(async move {
                let update_channel = async move { Channel::<T, S>::new(conn).await };

                let global_mutex = async move {
                    NetMutex::<u64, S>::new(
                        conn,
                        if conn.node_type() == RelativeNodeType::Initiator {
                            Some(0)
                        } else {
                            None
                        },
                ...
                Ok(Self {
                    global,
                    inner: Arc::new(RwLock::new(C::default())),
...
mod loader {
    ...
    > {
        pub(crate) inner: Pin<
            Box<
                dyn Future<Output = Result<NetAbstractCollection<T, K, C, S>, anyhow::Error>>
                    + Send
                    + 'a,
            >,
        >,
    ...
    impl<T: NetObject, K, C: AbstractCollection<K, NetRwLock<T, S>>, S: Subscribable + 'static>
        Future for NetVecLoader<'_, T, K, C, S>
    {
