...
use crate::hdp::hdp_packet::{packet_flags, HdpPacket};
...
use crate::hdp::peer::peer_layer::{HyperNodePeerLayer, PeerSignal, UdpMode};
use crate::hdp::session_queue_handler::{
        ...
        match state {
            SessionState::NeedsRegister => {
                ...
                let session_security_settings =
                    state_container.session_security_settings.clone().unwrap();
                let proposed_username = state_container
                    .connect_state
                    .proposed_credentials
                    .as_ref()
                    .ok_or_else(|| NetworkError::InternalError("Proposed credentials not loaded"))?
                    .username();
                ...
                let passwordless = state_container
                    ...
                    .passwordless
                    .clone()
                    .ok_or_else(|| NetworkError::InternalError("Passwordless state not loaded"))?;
                ...
                let stage0_register_packet =
                    crate::hdp::hdp_packet_crafter::do_register::craft_stage0(
...
impl HdpSessionInner {
    ...
    pub(super) fn create_welcome_message(&self, cid: u64) -> String {
        format!(
            "LUSNA login::success. Welcome to the Post-quantum network. Implicated CID: {}",
            cid
        )
    }
