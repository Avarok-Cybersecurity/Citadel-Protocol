...
impl TransferStats {
    ...
    pub fn new(timestamp: i64, plaintext_bytes_sent: isize) -> Self {
        Self { timestamp, plaintext_bytes_in_window: plaintext_bytes_sent, transfer_rate: 0f32, transfer_rate_jitter: 0f32, total_plaintext_bytes_sent: 0 }
    }
}
...
impl AddAssign for TransferStats {
    fn add_assign(&mut self, rhs: Self) {
        ...
        let transfer_rate_jitter: f64 = ((rhs.transfer_rate - self.transfer_rate) as f64) / diff_sec;
        let transfer_rate: f64 = (rhs.plaintext_bytes_in_window - self.plaintext_bytes_in_window) as f64 / diff_sec;
        self.transfer_rate = transfer_rate as f32;
        self.transfer_rate_jitter = transfer_rate_jitter as f32;

        self.timestamp = rhs.timestamp;
        self.total_plaintext_bytes_sent = self.total_plaintext_bytes_sent.wrapping_add(rhs.plaintext_bytes_in_window);
