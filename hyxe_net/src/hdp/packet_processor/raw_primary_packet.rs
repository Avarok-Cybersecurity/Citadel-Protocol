...
pub fn process_raw_packet(this_implicated_cid: Option<u64>, session: &HdpSession, remote_peer: SocketAddr, local_primary_port: u16, packet: BytesMut, concurrent_processor_tx: &ConcurrentProcessorTx) -> Result<PrimaryProcessorResult, NetworkError> {
        ...
        Some(packet) => {
            match cmd_primary {
                packet_flags::cmd::primary::DO_REGISTER => {
                    super::register_packet::process_register(session, packet, remote_peer, concurrent_processor_tx)
                }

                packet_flags::cmd::primary::DO_CONNECT => {
                    super::connect_packet::process_connect(session, packet, concurrent_processor_tx)
                }

                packet_flags::cmd::primary::KEEP_ALIVE => {
                    super::keep_alive_packet::process_keep_alive(session, packet, concurrent_processor_tx)
                }
                ...
                packet_flags::cmd::primary::DO_DEREGISTER =>  {
                    super::deregister_packet::process_deregister(session, packet, concurrent_processor_tx)
                }

                packet_flags::cmd::primary::DO_PRE_CONNECT => {
                    super::preconnect_packet::process_preconnect(session, packet, concurrent_processor_tx)
                }

                packet_flags::cmd::primary::PEER_CMD => {
                    peer_cmd_packet::process_peer_cmd(session, cmd_aux, packet, header_drill_vers, endpoint_cid_info, concurrent_processor_tx)
                }
