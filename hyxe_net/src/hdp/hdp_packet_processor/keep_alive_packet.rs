...
pub fn process(session: &HdpSession, packet: HdpPacket) -> PrimaryProcessorResult {
    ...
    if let Some((header,_payload, hyper_ratchet)) = validation::keep_alive::validate_keep_alive(cnac, &header, payload) {
        ...
        if state_container.on_keep_alive_received(header.timestamp.get(), current_timestamp_ns) {
            ...
            let hyper_ratchet = cnac.get_hyper_ratchet(Some(hyper_ratchet.version().wrapping_add(1))).unwrap_or(hyper_ratchet);
            let future = async move {
                ...
                let next_ka = hdp_packet_crafter::keep_alive::craft_keep_alive_packet(&hyper_ratchet, current_timestamp_ns + DELTA_NS);
