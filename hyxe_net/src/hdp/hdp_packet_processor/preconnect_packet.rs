        ...
        packet_flags::cmd::aux::do_preconnect::SYN => {
            ...
            if let Some(cnac) = account_manager.get_client_by_cid(header.session_cid.get()).await? {
                ...
                match validation::pre_connect::validate_syn(&cnac, packet, &session.session_manager) {
                    Ok((static_aux_ratchet, transfer, session_security_settings, peer_only_connect_mode, udp_mode, kat)) => {
                    ...
                    match LinearUDPHolePuncher::reserve_new_udp_sockets(wave_port_count, local_bind_ip) {
                        Ok(reserved_sockets) => {
                            ...
                            if initial_traversal_method == NatTraversalMethod::UPnP {
                                ...
                                handle_nat_traversal_as_receiver(session_orig.clone(), hyper_ratchet, initial_traversal_method, sync_time_instant, endpoints, reserved_sockets, security_level, VirtualTargetType::HyperLANPeerToHyperLANServer(cnac.get_cid()));
                            }
        ...
        packet_flags::cmd::aux::do_preconnect::STAGE1 => {
            ...
            if state_container.pre_connect_state.last_stage == packet_flags::cmd::aux::do_preconnect::STAGE1 {
                if let Some((adjacent_node_type, proposed_traversal_method, sync_time, base_wave_ports)) = validation::pre_connect::validate_stage1(&hyper_ratchet, packet) {
                    ...
                    handle_nat_traversal_as_initiator(session_orig.clone(), hyper_ratchet, proposed_traversal_method, sync_time, endpoints, reserved_sockets, security_level, VirtualTargetType::HyperLANPeerToHyperLANServer(cnac.get_cid()));
                    PrimaryProcessorResult::Void
                } else {
        ...
        packet_flags::cmd::aux::do_preconnect::STAGE_TRY_NEXT => {
            ...
            if this_node_last_stage == packet_flags::cmd::aux::do_preconnect::SUCCESS || this_node_last_stage == packet_flags::cmd::aux::do_preconnect::FAILURE {
                if let Some((hyper_ratchet, next_traversal_method)) = validation::pre_connect::validate_try_next(cnac, packet) {
                    ...
                    handle_nat_traversal_as_receiver(session_orig.clone(), hyper_ratchet, next_traversal_method, sync_time_instant, endpoints, preserved_sockets, security_level, VirtualTargetType::HyperLANPeerToHyperLANServer(cnac.get_cid()));
...
fn handle_nat_traversal_as_receiver(session: HdpSession, hyper_ratchet: HyperRatchet, method: NatTraversalMethod, sync_time: Instant, endpoints: Vec<SocketAddr>, sockets: Vec<UdpSocket>, security_level: SecurityLevel, v_target: VirtualTargetType) {
    spawn!(handle_nat_traversal_as_receiver_inner(session, hyper_ratchet, method, sync_time, endpoints, sockets, security_level, v_target));
}

#[allow(unused_results)]
async fn handle_nat_traversal_as_receiver_inner(session_orig: HdpSession, hyper_ratchet: HyperRatchet, method: NatTraversalMethod, sync_time: Instant, endpoints: Vec<SocketAddr>, mut sockets: Vec<UdpSocket>, security_level: SecurityLevel, v_target: VirtualTargetType) {
    let mut hole_puncher = {
        ...
        let local_node_type = session.local_node_type;
        let hole_puncher = LinearUDPHolePuncher::new_receiver(local_node_type, crypt_container);
    ...
    match hole_puncher.try_method(&mut sockets, &endpoints, method).await {
        Ok(set) => {
            ...
            let mut set = sockets.into_iter().zip(set.into_iter()).collect::<Vec<(UdpSocket, HolePunchedSocketAddr)>>();
            let (socket, addr) = set.remove(0);
...
#[allow(unused_results)]
fn handle_nat_traversal_as_initiator(session: HdpSession, hyper_ratchet: HyperRatchet, method: NatTraversalMethod, sync_time: Option<Instant>, endpoints: Vec<SocketAddr>, sockets: Vec<UdpSocket>, security_level: SecurityLevel, v_target: VirtualTargetType) {
    spawn!(handle_nat_traversal_as_initiator_inner(session, hyper_ratchet, method, sync_time, endpoints, sockets, security_level, v_target));
}

#[allow(unused_results)]
async fn handle_nat_traversal_as_initiator_inner(session_orig: HdpSession, hyper_ratchet: HyperRatchet, method: NatTraversalMethod, sync_time: Option<Instant>, endpoints: Vec<SocketAddr>, mut sockets: Vec<UdpSocket>, security_level: SecurityLevel, v_target: VirtualTargetType) {
    let mut hole_puncher = {
        ...
        let crypt_container = generate_hole_punch_crypt_container(hyper_ratchet.clone(), security_level);
        let local_node_type = session.local_node_type;
        let hole_puncher = LinearUDPHolePuncher::new_initiator(local_node_type, crypt_container);
        //std::mem::drop(session);
        hole_puncher
    };
    ...
    match hole_puncher.try_method(&mut sockets, &endpoints, method).await {
        Ok(set) => {
            ...
            if method != NatTraversalMethod::UPnP {
                ...
                match send_success_as_initiator(set, method, &hyper_ratchet, &sess, security_level, v_target) {
                    PrimaryProcessorResult::ReplyToSender(packet) => {
...
fn send_success_as_initiator(mut set: Vec<(UdpSocket, HolePunchedSocketAddr)>, method: NatTraversalMethod, hyper_ratchet: &HyperRatchet, session: &HdpSession, security_level: SecurityLevel, v_target: VirtualTargetType) -> PrimaryProcessorResult {
