...
use crate::hdp::peer::peer_layer::Username;

pub fn handle_response_phase(peer_conn_type: PeerConnectionType, username: Username, peer_response: PeerResponse, ticket: Ticket, fcm: FcmPostRegister, implicated_cid: u64, target_cid: u64, timestamp: i64, session: &HdpSession, sess_hyper_ratchet: &HyperRatchet, security_level: SecurityLevel) -> Result<PrimaryProcessorResult, NetworkError> {
    let decline = match &peer_response { PeerResponse::Decline => true, _ => false };

    route_signal_response(PeerSignal::PostRegister(peer_conn_type, username, None,Some(ticket), Some(peer_response), fcm), implicated_cid, target_cid, timestamp, ticket, session.clone(), &sess_hyper_ratchet,
