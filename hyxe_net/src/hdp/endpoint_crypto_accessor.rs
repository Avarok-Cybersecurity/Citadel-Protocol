...
impl EndpointCryptoAccessor {
    ...
    pub fn borrow_hr<F, T>(&self, vers: Option<u32>, access: F) -> Result<T, NetworkError>
        where F: for<'a> FnOnce(&'a StackedRatchet, &mut dyn ExpectedInnerTargetMut<StateContainerInner>) -> T {
        ...
        match self {
            Self::P2P(peer_cid, state_container) => {
                ...
                v_conn.endpoint_crypto.get_hyper_ratchet(vers).cloned().ok_or(NetworkError::InternalError("P2P HR does not exist")).map(|r| access(&r, &mut state_container))
            }
