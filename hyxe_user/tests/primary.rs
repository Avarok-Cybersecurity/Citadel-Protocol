...
mod tests {
    ...
    use futures::Future;
    use hyxe_crypt::prelude::{ConstructorOpts, SecBuffer};
    ...
    use hyxe_user::prelude::{ConnectionInfo, MutualPeer};
        ...
        F: Future<Output = Result<(), AccountError>> + Send + 'static,
    {
        ...
        async fn harness_inner<T, F>(
            client_backend: &BackendType,
            server_backend: &BackendType,
            t: &mut T,
        ) -> Result<(), AccountError>
        where
            T: Send + 'static + FnMut(TestContainer, PersistenceHandler, PersistenceHandler) -> F,
            F: Future<Output = Result<(), AccountError>> + Send + 'static,
        {
            ...
            let (pers_cl, pers_se) = (
                container.client_acc_mgr.get_persistence_handler().clone(),
    ...
    async fn test_interload_persistence() -> Result<(), AccountError> {
        ...
        for server_backend in server_backends {
            ...
            if !matches!(server_backend, BackendType::InMemory) {
                ...
                let cont_reloaded =
                    TestContainer::new(server_backend.clone(), BackendType::InMemory).await;
                assert_eq!(
                    cont_reloaded
                        .client_acc_mgr
                        .get_persistence_handler()
                        ...
                        .len(),
                    0
                ); // since in-memory does not persist
                assert_eq!(
                    cont_reloaded
                        .server_acc_mgr
                        .get_persistence_handler()
        ...
        for client_backend in client_backends {
            ...
            if !matches!(client_backend, BackendType::InMemory) {
                ...
                let cont_reloaded =
                    TestContainer::new(BackendType::InMemory, client_backend.clone()).await;
                assert_eq!(
                    cont_reloaded
                        .client_acc_mgr
                        .get_persistence_handler()
    ...
    async fn test_cnac_creation() -> Result<(), AccountError> {
        test_harness(|container, pers_cl, pers_se| async move {
            ...
            assert_eq!(
                pers_cl
                    .get_client_metadata(client.get_cid())
                    .await
                    .unwrap()
                    .unwrap(),
                CNACMetadata {
            ...
            assert_eq!(
                pers_cl.get_clients_metadata(None).await.unwrap(),
                vec![CNACMetadata {
                    cid: client.get_cid(),
                    username: USERNAME.to_string(),
                    full_name: FULL_NAME.to_string(),
            ...
            assert_eq!(
                pers_se.get_clients_metadata(None).await.unwrap(),
                vec![CNACMetadata {
                    cid: client.get_cid(),
                    username: USERNAME.to_string(),
                    full_name: FULL_NAME.to_string(),
    ...
    async fn test_byte_map() -> Result<(), AccountError> {
        test_harness(|container, pers_cl, pers_se| async move {
            ...
            assert!(pers_cl
                .store_byte_map_value(client.get_cid(), 1234, "thekey", "sub_key", dummy.clone())
                .await
                .unwrap()
                .is_none());
            assert_eq!(
                pers_cl
                    .get_byte_map_value(client.get_cid(), 1234, "thekey", "sub_key")
                    .await
                    .unwrap()
                    .unwrap(),
                dummy.clone()
            ...
            assert_eq!(
                pers_cl
                    .get_byte_map_values_by_key(client.get_cid(), 1234, "thekey")
                    .await
                    .unwrap()
                    .remove("sub_key")
                    .unwrap(),
            ...
            assert_eq!(
                pers_cl
                    .remove_byte_map_value(client.get_cid(), 1234, "thekey", "sub_key")
                    .await
                    .unwrap()
                    .unwrap(),
                dummy.clone()
            ...
            assert_eq!(
                pers_se
                    .get_byte_map_value(server.get_cid(), 1234, "helloworld", "sub_key")
                    .await
                    .unwrap()
                    .unwrap(),
                dummy.clone()
            ...
            assert_eq!(
                pers_se
                    .get_byte_map_values_by_key(server.get_cid(), 1234, "helloworld")
                    .await
                    .unwrap()
                    .remove("sub_key")
                    .unwrap(),
    ...
    async fn test_byte_map2() -> Result<(), AccountError> {
        test_harness(|container, pers_cl, _pers_se| async move {
            ...
            let dummy = Vec::from("Hello, world!");
            assert!(pers_cl
                .store_byte_map_value(client.get_cid(), 1234, "thekey", "sub_key1", dummy.clone())
                .await
                .unwrap()
                .is_none());
            assert!(pers_cl
                .store_byte_map_value(client.get_cid(), 1234, "thekey", "sub_key2", dummy.clone())
                .await
                .unwrap()
                .is_none());
            assert!(pers_cl
                .store_byte_map_value(client.get_cid(), 1234, "thekey", "sub_key3", dummy.clone())
                .await
                .unwrap()
                .is_none());
            assert!(pers_cl
            ...
            let del_map = pers_cl
                .remove_byte_map_values_by_key(client.get_cid(), 1234, "thekey")
                .await
                .unwrap();
            ...
            assert_eq!(
                pers_cl
                    .get_byte_map_value(client.get_cid(), 1234, "unrelated", "sub_key4")
                    .await
                    .unwrap()
                    .unwrap(),
                dummy.clone()
            );
            let del_map2 = pers_cl
                .remove_byte_map_values_by_key(client.get_cid(), 1234, "thekey")
                .await
                .unwrap();
    ...
    async fn test_cnac_meta() -> Result<(), AccountError> {
        test_harness(|container, pers_cl, pers_se| async move {
            ...
            assert_eq!(
                pers_se
                    .get_username_by_cid(client.get_cid())
                    .await
                    .unwrap()
                    .unwrap(),
                USERNAME
    ...
    async fn test_register_p2p() -> Result<(), AccountError> {
        test_harness(|container, pers_cl, pers_se| async move {
            ...
            assert_eq!(
                pers_cl
                    .get_hyperlan_peer_list(client.get_cid())
                    .await
                    .unwrap()
                    .unwrap(),
                vec![peer_cnac.get_cid()]
            );
            assert_eq!(
                peer_pers
                    .get_hyperlan_peer_list(peer_cnac.get_cid())
                    .await
                    .unwrap()
                    .unwrap(),
                vec![client.get_cid()]
            ...
            assert_eq!(
                pers_cl
                    .get_hyperlan_peer_by_cid(client.get_cid(), peer_cnac.get_cid())
                    .await
                    .unwrap()
                    .unwrap(),
                MutualPeer {
            ...
            assert!(peer_pers
                .hyperlan_peer_exists(peer_cnac.get_cid(), client.get_cid())
                .await
                .unwrap());
            assert!(pers_cl
                .hyperlan_peer_exists(client.get_cid(), peer_cnac.get_cid())
                .await
                .unwrap());
            assert!(pers_se
                .hyperlan_peer_exists(peer_cnac.get_cid(), client.get_cid())
    ...
    async fn test_deregister_p2p() -> Result<(), AccountError> {
        test_harness(|container, pers_cl, pers_se| async move {
            ...
            assert!(peer_pers
                .get_hyperlan_peer_by_username(peer_cnac.get_cid(), USERNAME)
                .await
                .unwrap()
                .is_none());
            assert!(pers_cl
                .get_hyperlan_peer_by_username(client.get_cid(), peer.0.as_str())
                .await
                .unwrap()
                .is_none());
            assert!(pers_se
    ...
    async fn test_deregister_client_from_server() -> Result<(), AccountError> {
        test_harness(|container, pers_cl, pers_se| async move {
            ...
            assert!(pers_cl
                .get_cnac_by_cid(client.get_cid())
                .await
                .unwrap()
                .is_none());
            assert!(pers_se
                ...
                .is_none());
            assert!(pers_se
                .get_cnac_by_cid(peer_cnac.get_cid())
                .await
                .unwrap()
                .is_some());
            assert!(pers_se
        ...
        test_harness(|container, pers_cl, pers_se| async move {
            ...
            for peer in PEERS.iter() {
                let (peer_cnac, peer_container) = container
                    ...
                    .await;
                assert!(peer_map
                    .insert(peer.0.to_string(), peer_cnac.get_cid())
                    .is_none());
                let peer_pers = &peer_container
                    .client_acc_mgr
                    .get_persistence_handler()
    ...
    async fn test_register_p2p_many_list() -> Result<(), AccountError> {
        test_harness(|container, pers_cl, pers_se| async move {
            ...
            let client_peers = pers_cl
                .get_hyperlan_peer_list(client.get_cid())
                .await
                .unwrap()
                .unwrap();
            assert_eq!(client_peers.len(), PEERS.len());
            let impersonals = pers_se
                .get_registered_impersonal_cids(None)
                .await
                .unwrap()
                .unwrap();
    ...
    async fn register_peers(
        peer0_pers: &PersistenceHandler,
        peer0_cid: u64,
        peer0_username: &str,
        peer1_pers: &PersistenceHandler,
        peer1_cid: u64,
        peer1_username: &str,
        server_pers: &PersistenceHandler,
    ) {
        peer0_pers
            .register_p2p_as_client(peer0_cid, peer1_cid, peer1_username.to_string())
            .await
            .unwrap();
        peer1_pers
            .register_p2p_as_client(peer1_cid, peer0_cid, peer0_username.to_string())
            .await
            .unwrap();
        server_pers
            .register_p2p_as_server(peer0_cid, peer1_cid)
            .await
            .unwrap();
    }
    ...
    async fn deregister_peers(
        ...
        peer0_pers
            .deregister_p2p_as_client(peer0_cid, peer1_cid)
            .await
            .unwrap();
        peer1_pers
            .deregister_p2p_as_client(peer1_cid, peer0_cid)
            .await
            .unwrap();
        server_pers
            .deregister_p2p_as_server(peer0_cid, peer1_cid)
            .await
            .unwrap();
    }

    async fn deregister_client_from_server(
        pers_cl: &PersistenceHandler,
        client_cid: u64,
        server_pers: &PersistenceHandler,
    ) {
