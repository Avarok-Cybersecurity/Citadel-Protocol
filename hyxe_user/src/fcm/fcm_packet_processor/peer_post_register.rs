...
pub enum InvitationType {
    PostRegister(FcmPostRegister, String)
}
...
pub fn process(post_register_store: &mut HashMap<u64, InvitationType>, local_cid: u64, transfer: FcmPostRegister, username: String) -> FcmProcessorResult {
    // store inside cnac
    let peer_cid = transfer.get_peer_cid()?;
    if let Some(_) = post_register_store.insert(peer_cid, InvitationType::PostRegister(transfer, username.clone())) {
        log::warn!("Overwrote pre-existing invite request. Previous is thus invalidated");
    }

    // finally, return signal to caller
    FcmProcessorResult::Value(FcmResult::PostRegisterInvitation { invite: PostRegisterInvitation { peer_cid, local_cid, username: username.into_bytes() } })
}