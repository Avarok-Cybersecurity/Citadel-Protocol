...
impl ExternalServiceChannel for RtdbInstance {
    async fn send(&mut self, data: RawExternalPacket, implicated_cid: u64, peer_cid: u64) -> Result<(), AccountError> {
        ...
        Ok(self.root().await.map_err(|err| AccountError::Generic(err.inner))?.child("users").child(peer_cid.to_string()).child("peers").child(implicated_cid.to_string()).final_node("packets").post(data).await.map(|_| ()).map_err(|err| AccountError::Generic(err.inner))?)
    }
}