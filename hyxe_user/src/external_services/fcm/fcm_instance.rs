...
impl FCMInstance {
    ...
    pub async fn send_message_to_with_opts<T: AsRef<str>, Z: Serialize>(&self, target_reg_id: T, data: Z, cfg: impl FnOnce(&mut MessageBuilder<'_>)) -> Result<FcmResponse, AccountError> {
        Self::send_to_fcm_user_inner(self.api_key(), target_reg_id.as_ref(), self.client(), data, cfg).await
    }

    pub async fn send_message_to<T: AsRef<str>, Z: Serialize>(&self, target_reg_id: T, data: Z) -> Result<FcmResponse, AccountError> {
    ...
    pub async fn send_to_fcm_user<T: Serialize>(&self, data: T) -> Result<FcmResponse, AccountError> {
        self.send_message_to(self.client_id(), data).await
    }

    pub async fn send_to_fcm_user_by_value<T: Serialize>(self, data: T) -> Result<FcmResponse, AccountError> {
        self.send_message_to(self.client_id(), data).await
    }
    ...
    pub(crate) fn map_fcm_resp(resp: Result<FcmResponse, FcmError>) -> Result<FcmResponse, AccountError> {
    ...
    #[allow(unused_results)]
    pub async fn broadcast_to_users<T: Serialize, V: AsRef<[String]>>(&self, data: T, users: V) -> Result<FcmResponse, AccountError> {
