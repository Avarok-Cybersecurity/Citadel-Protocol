...
class Message extends AbstractSqlObject {
  ...
  static const String GENESIS = "CREATE TABLE $DB_TABLE (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, implicatedCid TEXT, peerCid TEXT, message TEXT, lastEventTime TEXT, initTime TEXT, fromPeer INTEGER, status TEXT, rawTicket TEXT)";
  ...
  Message(this.implicatedCid, this.peerCid, this.message, this.lastEventTime, this.fromPeer, this.status, this.rawTicket) : this.initTime = lastEventTime;
  ...
  this.initTime = DateTime.parse(sql["initTime"]),
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'initTime': this.initTime.toIso8601String(),
  ...
  Future<void> update() async {
    if (this._id != null) {
      var messageOpt = await DatabaseHandler.getObjectByID(DB_TABLE, this._id, (map) => Message.fromMap(map));
      if (messageOpt.isPresent) {
        var message = messageOpt.value;
        this.rawTicket = message.rawTicket;
        this.status = message.status;
        this.lastEventTime = message.lastEventTime;
      }
    }
  }
  ...
  static Future<Optional<Message>> getMessage(u64 implicatedCid, u64 peerCid, u64 rawTicket, {bool localRecvOnly = false}) async {
    if (localRecvOnly) {
      var db = await DatabaseHandler.database();
      var list = await db.rawQuery("SELECT * FROM $DB_TABLE WHERE implicatedCid = '$implicatedCid' AND peerCid = '$peerCid' AND rawTicket = '$rawTicket' AND fromPeer = 1 LIMIT 1");
      if (list.isNotEmpty) {
