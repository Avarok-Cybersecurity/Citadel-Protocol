...
class PeerSendHandler extends AbstractHandler {
  ...
  PeerSendHandler._(this.onStatusUpdateReceived, this.message);
  PeerSendHandler.screen(void Function(PeerSendUpdate) onStatusUpdateReceived, Message message) : this._(Optional.of(onStatusUpdateReceived), message);
  PeerSendHandler.screenless(Message message) : this._(Optional.empty(), message);
  ...
  CallbackStatus onConfirmation(KernelResponse kernelResponse) {
    ...
    this.onStatusUpdateReceived.ifPresent((fx) => fx.call(PeerSendUpdate(PeerSendState.MessageSent, kernelResponse.getTicket(), this.message)));
  ...
  void onErrorReceived(ErrorKernelResponse kernelResponse) {
    ...
    this.onStatusUpdateReceived.ifPresent((fx) => fx.call(PeerSendUpdate(PeerSendState.Failure, kernelResponse.getTicket(), this.message)));
  }
  ...
  Future<CallbackStatus> onTicketReceived(KernelResponse kernelResponse) async {
    if (kernelResponse is MessageReceived) {
      ...
      this.onStatusUpdateReceived.ifPresent((fx) => fx.call(PeerSendUpdate(PeerSendState.MessageReceived, kernelResponse.getTicket(), this.message)));
      ...
      if (AbstractHandler.validTypes(kernelResponse, DomainSpecificResponseType.FcmMessageReceived)) {
        ...
        await this.updateMessageState(PeerSendState.MessageReceived);
        await MessageSendHandler.pollSpecificChannel(message);
        this.onStatusUpdateReceived.ifPresent((fx) => fx.call(PeerSendUpdate(PeerSendState.MessageReceived, kernelResponse.getTicket(), this.message)));
        return CallbackStatus.Complete;
      } else {
