...
pub fn load_and_execute_ffi_static(port: i64, home_dir: &str, database: &str) -> i32 {
    ...
    if let Some(_) = old_opt {
        log::warn!(target: "citadel", "FFI_STATIC has been replaced! Check the calling program's logic");
...
impl FFIObject {
    ...
    {
        Arc::new(Box::new(|res| {
            log::trace!(target: "citadel", "About to send {:?}", &res);
            ...
            {
                true => {
                    log::trace!(target: "citadel", "Successfully sent FFI Message")
                }

                false => {
                    log::error!(target: "citadel", "Unable to send to dart! [FATAL]");
                }
            }
        }))
    }
    ...
    fn execute_once(&self, home_dir: &str, database: &str) {
        ...
        std::thread::spawn(move || {
            log::trace!(target: "citadel", "Started SatoriNET main thread ...");
            if let Err(err) =
                hyxewave::ffi::ffi_entry::execute_lusna_kernel(config, to_ffi_frontier.clone())
            {
                log::error!(target: "citadel", "Err executing kernel: {:?}", &err);
