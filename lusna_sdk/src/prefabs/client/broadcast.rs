...
pub enum GroupInitRequestType {
    ...
    Create { local_user: UserIdentifier, invite_list: Vec<UserIdentifier>, group_id: Uuid, accept_registrations: bool },
    ...
    Join { local_user: UserIdentifier, owner: UserIdentifier, group_id: Uuid, do_peer_register: bool },
}
...
impl<'a, F, Fut> PrefabFunctions<'a, GroupInitRequestType> for BroadcastKernel<'a, F, Fut>
    ...
    async fn on_c2s_channel_received(connect_success: ConnectSuccess, mut remote: ClientServerRemote, arg: GroupInitRequestType, fx: Self::UserLevelInputFunction, shared: Arc<BroadcastShared>) -> Result<(), NetworkError> {
        ...
        let request = match arg {
            GroupInitRequestType::Create { local_user, invite_list, group_id, accept_registrations } => {
                ...
                for peer in &invite_list {
                    let peer = peer.search_peer(implicated_cid, remote.inner.account_manager()).await?
                        .ok_or_else(|| NetworkError::msg(format!("[create] User {:?} is not registered to {:?}", peer, &local_user)))?;
                    peers_registered.push(peer.cid)
                }

                creator_only_accept_inbound_registers = accept_registrations;

                GroupBroadcast::Create(peers_registered, MessageGroupOptions { group_type: GroupType::Public, id: group_id.as_u128() })
            }

            GroupInitRequestType::Join { local_user, owner, group_id, do_peer_register } => {
                // ensure local is registered to owner
                let owner_orig = owner;
                let owner_find = owner_orig.search_peer(implicated_cid, remote.inner.account_manager()).await?;
                let owner = if let Some(owner) = owner_find {
                    Some(owner)
                } else {
                    if do_peer_register {
                        let mut handle = remote.inner.propose_target(local_user.clone(), owner_orig.clone()).await?;
                        let _ = handle.register_to_peer().await?;
                        owner_orig.search_peer(implicated_cid, remote.inner.account_manager()).await?
                    } else {
                        None
                    }
                };

                let owner = owner.ok_or_else(|| NetworkError::msg(format!("User {:?} is not registered to {:?}", owner_orig, &local_user)))?;

                GroupBroadcast::RequestJoin(MessageGroupKey { cid: owner.cid, mgid: group_id.as_u128() })
            }
        };
        ...
        let acceptor_task = if creator_only_accept_inbound_registers {
            ...
            Box::pin(async move {
                ...
                Ok::<_, NetworkError>(())
            }) as Pin<Box<dyn futures::Future<Output = Result<(), hyxe_net::prelude::NetworkError>> + Send>>
        } else {
            Box::pin(async move { Ok::<_, NetworkError>(()) }) as Pin<Box<dyn futures::Future<Output = Result<(), hyxe_net::prelude::NetworkError>> + Send>>
        };
...
mod tests {
    ...
    use crate::test_common::{server_info, TestBarrier, wait_for_peers};
    use uuid::Uuid;
