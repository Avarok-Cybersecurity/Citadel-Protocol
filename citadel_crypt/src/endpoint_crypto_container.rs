...
impl<R: Ratchet> PeerSessionCrypto<R> {
    ...
    > {
        ...
        newest_version.update_version(next_vers).ok_or_else(|| {
            ...
            CryptError::rekey_update("Unable to progress past update_version".to_string())
        })?;
        ...
        let transfer = newest_version.stage0_bob().ok_or_else(|| {
            CryptError::rekey_update("Unable to progress past stage0_bob".to_string())
        })?;
        ...
        let status = {
            ...
            toolset.update_from(next_ratchet).ok_or_else(|| {
                CryptError::rekey_update("Unable to progress past update_from".to_string())
            })?
        };
