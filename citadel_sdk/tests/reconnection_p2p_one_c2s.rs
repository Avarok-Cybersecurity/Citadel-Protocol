...
//! When B's C2S disconnects, A should receive a P2P disconnect signal.
//!
//! ## Causal Pathway
//!
//! ```text
...
//! send P2P messages → B.disconnect() [C2S] →
//! A receives PeerSignal::Disconnect → B: C2S Connect → rekey() →
...
//! |-------|-------|--------|-----------------|
//! | 2 | B | disconnect() (C2S) | A receives P2P disconnect |
//! | 3 | B | reconnect C2S | - |
...
mod tests {
    ...
    async fn test_p2p_after_one_c2s_disconnect() {
        ...
        let username_a = format!("p2a_{}", &uuid_a.to_string()[..8]);
        let username_b = format!("p2b_{}", &uuid_b.to_string()[..8]);
        ...
        let client_a_kernel = ReconnectionTestKernel::new(
            ...
            move |remote: NodeRemote<StackedRatchet>, state: Arc<NodeState>| {
                ...
                async move {
                    ...
                    // ===== PHASE 3: Wait for B to reconnect, then reconnect P2P =====
                    state.set_phase(3);
                    barrier3.wait().await;
                    ...
                    // A should have received at least 1 P2P disconnect signal when B's C2S dropped.
                    // Additional cleanup disconnects may arrive.
                    state.assert_final_state_with_min_p2p_disconnect(
                        "Peer A (Test 2)",
                        0,  // c2s_connect (intercepted)
                        0,  // c2s_disconnect (A's C2S stays active)
                        0,  // p2p_connect (intercepted)
                        1,  // min p2p_disconnect_recv (from B's C2S disconnect)
                        10, // msgs_sent
                        10, // msgs_recv
                    );
        ...
        let client_b_kernel = ReconnectionTestKernel::new(
            ...
            move |remote: NodeRemote<StackedRatchet>, state: Arc<NodeState>| {
                ...
                async move {
                    ...
                    // Drop P2P refs before C2S disconnect
                    drop(tx);
                    drop(rx);
                    ...
                    // Reconnect C2S (login, not register)
                    let conn2 = remote
                        .connect_with_defaults(AuthenticationRequest::credentialed(
                            username.clone(),
                            password,
                    ...
                    // B disconnected C2S once and reconnected.
                    // B initiated, so may receive 0+ cleanup disconnects.
                    state.assert_final_state_with_min_p2p_disconnect(
                        "Peer B (Test 2)",
                        0,  // c2s_connect (intercepted)
                        0,  // c2s_disconnect (intercepted by disconnect() call)
                        0,  // p2p_connect (intercepted)
                        0,  // min p2p_disconnect_recv (B initiated via C2S drop)
                        10, // msgs_sent
                        10, // msgs_recv
                    );
