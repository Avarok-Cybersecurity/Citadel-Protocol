...
mod tests {
    ...
    #[citadel_io::tokio::test(flavor = "multi_thread")]
    async fn test_channel_into_sink_round_trip() {
        citadel_logging::setup_log();
        citadel_sdk::test_common::TestBarrier::setup(2);
        ...
        let uuid = Uuid::new_v4();
        let username = format!("snk_{}", &uuid.to_string()[..8]);
        let password = "password123";
        ...
        let client_kernel = citadel_sdk::prefabs::client::single_connection::SingleClientServerConnectionKernel::new(
            ...
            move |mut conn| async move {
                ...
                // Drive the socket-like Sink<Bytes> surface.
                let mut sink = tx.into_sink();
                for i in 0..COUNT {
                    sink.send(Bytes::from(expected_message(i))).await?;
                }
                ...
                for i in 0..COUNT {
                    ...
                    assert_eq!(
                        echoed.as_ref(),
                        expected_message(i).as_slice(),
                        "Sink<Bytes> payload {i} did not round-trip intact"
                    );
                }
