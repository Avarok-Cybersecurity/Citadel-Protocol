...
//! This is the most complex scenario - P2P down, then full C2S reconnect for both.
//!
//! ## Causal Pathway
//!
//! ```text
...
//! send P2P messages → P2P disconnect() → A.disconnect() → B.disconnect() →
//! A: C2S Connect → rekey() → B: C2S Connect → rekey() →
...
//! |-------|-------|--------|-----------------|
//! | 2 | A | p2p.disconnect() | B receives P2P disconnect |
//! | 2 | Both | disconnect() (C2S) | - |
//! | 3 | Both | reconnect C2S | - |
...
mod tests {
    ...
    async fn test_p2p_then_both_c2s_disconnect() {
        ...
        let username_a = format!("bta_{}", &uuid_a.to_string()[..8]);
        let username_b = format!("btb_{}", &uuid_b.to_string()[..8]);
        ...
        let client_a_kernel = ReconnectionTestKernel::new(
            ...
            move |remote: NodeRemote<StackedRatchet>, state: Arc<NodeState>| {
                ...
                async move {
                    ...
                    // A initiated P2P disconnect, so may receive 0+ cleanup disconnects.
                    state.assert_final_state_with_min_p2p_disconnect(
                        ...
                        0,  // min p2p_disconnect_recv (A initiated P2P disconnect)
                        10, // msgs_sent
                        10, // msgs_recv
                    );
        ...
        let client_b_kernel = ReconnectionTestKernel::new(
            ...
            move |remote: NodeRemote<StackedRatchet>, state: Arc<NodeState>| {
                ...
                async move {
                    ...
                    // B should have received at least 1 P2P disconnect from A's P2P disconnect.
                    ...
                    state.assert_final_state_with_min_p2p_disconnect(
                        ...
                        0,  // c2s_disconnect (intercepted)
                        0,  // p2p_connect (intercepted)
                        1,  // min p2p_disconnect_recv (from A's P2P disconnect)
                        10, // msgs_sent
                        10, // msgs_recv
                    );
