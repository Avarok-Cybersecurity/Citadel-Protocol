...
//! Group messages across more C2S rekeys than the toolset holds, against a server reached by
//! WebSocket URL (a native one, or the Durable Object via CITADEL_GROUP_ENDPOINT).
//!
//! Live, every session eventually died on a group send with "Ratchet missing for v_conn
//! LocalGroupServer ... latest_usable_version=4" while the toolset held 5..=9: group channels
//! send through the C2S container, and it stopped advancing at its first truncating round.
//!
...
//!   --test group_messages_survive_c2s_rekeys -- --ignored
//! ```
//!
...
mod tests {
    ...
    /// Past the ratchet window (MAX_RATCHETS_IN_MEMORY), so both sides truncate for real.
    const ROUNDS: u32 = 40;
    ...
    async fn run(endpoint: WebSocketEndpoint) {
            ...
            GroupTestKernel::new(
                move |remote: NodeRemote<StackedRatchet>, _events: Events| async move {
                    ...
                    for round in 0..ROUNDS {
                        ...
                        sync.wait().await;
                        channel
                            .send_message(SecBuffer::from(format!("rk{round}-x").into_bytes()))
                            .await?;
                        sync.wait().await;
                    }
                    drop(channel);
                ...
                move |remote: NodeRemote<StackedRatchet>, mut events: Events| async move {
                    ...
                    for round in 0..ROUNDS {
                        ...
                        let got = tokio::time::timeout(
                            ...
                            next_message_with_prefix(&mut channel, &format!("rk{round}-")),
                        )
                        .await;
                        assert!(got.is_ok(), "the member never received round {round}");
                        sync.wait().await;
                    }
                    drop(channel);
                    conn.shutdown_kernel().await
        ...
        let result = tokio::time::timeout(std::time::Duration::from_secs(900), clients)
            .await
    ...
    async fn group_messages_survive_c2s_rekeys_on_a_native_server() {
    ...
    async fn group_messages_survive_c2s_rekeys_on_an_external_server() {
        citadel_logging::setup_log();
