...
pub fn handle<'a>(matches: &ArgMatches<'a>, server_remote: &'a HdpServerRemote, ctx: &'a ConsoleContext, ffi_io: Option<FFIIO>) -> Result<Option<KernelResponse>, ConsoleError> {
    ...
    let adjacent_ip = adjacent_nac.get_addr_blocking(true).ok_or(ConsoleError::Default("Adjacent NAC does not have an IP address. Corrupt. Please remove CNAC"))?;

    let proposed_credentials = get_proposed_credentials(matches, ctx, username, adjacent_ip, security_level, cid, full_name)?;

    let request = HdpServerRequest::ConnectToHypernode(adjacent_ip, cid, proposed_credentials, security_level, None, None, Some(tcp_only));
