...
pub async fn execute_lusna_kernel<T: ToString>(execute_args: T, to_ffi_frontier: Box<dyn Fn(Result<Option<KernelResponse>, ConsoleError>) + Send + 'static>) -> Result<(), ConsoleError> {
    ...
    setup_shutdown_hook();
    let (cfg, account_manager) = parse_command_line_arguments_into_app_config(Some(execute_args.to_string()), Some(ffi_object)).await?;
