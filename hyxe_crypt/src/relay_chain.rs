...
impl<R: Ratchet> CryptoRelayChain<R> {
    ...
    pub fn new(relay_len: usize) -> Option<Self> {
        if relay_len < 2 {
            ...
            Some(Self {
                links: LinkedHashMap::with_capacity(relay_len),
                target_cid_list: None,
            })
        }
    }
    ...
    pub fn on_finish(&mut self) -> Option<()> {
        ...
        keys.take(self.links.len() - 2)
            .for_each(|key| target_cid_list.push(*key));
    ...
    pub fn encrypt<T: AsRef<[u8]>>(
        ...
        nonce_version: usize,
        header_len: usize,
        header_inscriber: impl Fn(&R, u64, &mut BytesMut),
    ) -> Option<BytesMut> {
        ...
        |mut acc, (idx, ((_cid, container), target_cid))| {
            ...
            log::trace!(target: "lusna", "At IDX {} using endpoint container {}. Target CID: {}", idx, _cid, target_cid);
            if idx != 0 {
