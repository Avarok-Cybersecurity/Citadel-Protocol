...
pub fn decode_config_buf<T: ?Sized + AsRef<[u8]>>(
    ...
    let bytes_written;
    {
        let buffer_slice = &mut buffer[starting_output_len..];
...
/// This function saves performance compared to the vanilla crate because it doesn't have to allocate to a new memory sector; it simply overwrites existing data
pub fn decode_config_slice_auto<T: ?Sized + AsMut<[u8]>>(
    input: &mut T,
    config: Config,
) -> Result<usize, DecodeError> {
    let input_bytes = input.as_mut();
...
/// Writes to `output` at the cursor's current location via put_u8, and truncates by the amount of bytes written if successfull
pub fn decode_config_bytes_auto(
    input: &mut BytesMut,
    config: Config,
) -> Result<usize, DecodeError> {
    let len = input.len();
    let input_bytes = &mut input[..len];

    decode_helper_auto(
        input_bytes,
        num_chunks_from_usize(len),
        config,
    ).and_then(|bytes_encoded| {
        input.truncate(bytes_encoded);
        Ok(bytes_encoded)
    })
}

/// For use in base64-fast
fn num_chunks_from_usize(input: usize) -> usize {
    input
...
fn decode_helper_auto(
    ...
    while leftover_bits_appended_to_buf < leftover_bits_ready_to_append {
        // `as` simply truncates the higher bits, which is what we want here
        let selected_bits = (leftover_bits >> (56 - leftover_bits_appended_to_buf)) as u8;
        input[output_index] = selected_bits;
        output_index += 1;
...
fn decode_chunk_auto(
    ...
    let mut accum: u64;
    let morsel = decode_table[input[0 + index_at_start_of_input] as usize];

    if morsel == tables::INVALID_VALUE {
        return Err(DecodeError::InvalidByte(index_at_start_of_input, input[0]));
    }
    //input[index_at_start_of_input + 7] = morsel;
    accum = (morsel as u64) << 58;

    let morsel = decode_table[input[1 + index_at_start_of_input] as usize];
    if morsel == tables::INVALID_VALUE {
        return Err(DecodeError::InvalidByte(
            index_at_start_of_input + 1,
            input[1],
    ...
    //input[index_at_start_of_input + 6] = morsel;
    accum |= (morsel as u64) << 52;

    let morsel = decode_table[input[2 + index_at_start_of_input] as usize];
    if morsel == tables::INVALID_VALUE {
        return Err(DecodeError::InvalidByte(
            index_at_start_of_input + 2,
            input[2],
    ...
    //input[index_at_start_of_input + 5] = morsel;
    accum |= (morsel as u64) << 46;

    let morsel = decode_table[input[3 + index_at_start_of_input] as usize];
    if morsel == tables::INVALID_VALUE {
        return Err(DecodeError::InvalidByte(
            index_at_start_of_input + 3,
            input[3],
    ...
    //input[index_at_start_of_input + 4] = morsel;
    accum |= (morsel as u64) << 40;

    let morsel = decode_table[input[4 + index_at_start_of_input] as usize];
    if morsel == tables::INVALID_VALUE {
        return Err(DecodeError::InvalidByte(
            index_at_start_of_input + 4,
            input[4],
    ...
    //input[index_at_start_of_input + 3] = morsel;
    accum |= (morsel as u64) << 34;

    let morsel = decode_table[input[5 + index_at_start_of_input] as usize];
    if morsel == tables::INVALID_VALUE {
        return Err(DecodeError::InvalidByte(
            index_at_start_of_input + 5,
            input[5],
    ...
    //input[index_at_start_of_input + 2] = morsel;
    accum |= (morsel as u64) << 28;

    let morsel = decode_table[input[6 + index_at_start_of_input] as usize];
    if morsel == tables::INVALID_VALUE {
        return Err(DecodeError::InvalidByte(
            index_at_start_of_input + 6,
            input[6],
    ...
    //input[index_at_start_of_input + 1] = morsel;
    accum |= (morsel as u64) << 22;

    let morsel = decode_table[input[7 + index_at_start_of_input] as usize];
    if morsel == tables::INVALID_VALUE {
        return Err(DecodeError::InvalidByte(
            index_at_start_of_input + 7,
            input[7],
    ...
    //input[index_at_start_of_input + 0] = morsel;
    accum |= (morsel as u64) << 16;

    BigEndian::write_u64(&mut input[output_index..output_index + 8], accum);

    Ok(())
}

#[inline(always)]
fn decode_chunk_auto_6byte(
    input: &mut [u8],
    index_at_start_of_input: usize,
    decode_table: &[u8; 256],
    output_slice: &mut [u8],
    ...
    BigEndian::write_u64(output_slice, accum);
...
fn decode_chunk_precise_auto(
    ...
    input[output_index..output_index + 6].copy_from_slice(&tmp_buf[0..6]);
