...
/// This function will write using put_u8 ... make sure that the cursor for the `output` is in the desired location before starting!
pub fn encode_config_bytes<T: ?Sized + AsRef<[u8]>>(
    input: &T,
    config: Config,
    output: &mut BytesMut,
    ...
    if let Some(encoded_size) = encoded_size(input_bytes.len(), config) {
        output.reserve(encoded_size);
        encode_with_padding_bytes(input_bytes, config, encoded_size, output);
        return Ok(encoded_size);
    }

    Err(std::io::Error::new(std::io::ErrorKind::InvalidData, "Unable to use base64 encoding"))
}


/// Make sure to run encoded_size function beforehand to set the `output` size!
/// The output's size should have already reserved the bytes necessary
pub unsafe fn encode_config_bytes_unchecked<T: ?Sized + AsRef<[u8]>>(
    input: &T,
    config: Config,
    output: &mut BytesMut,
) -> usize {
...
/// For BufMut's
fn encode_with_padding_bytes(input: &[u8], config: Config, encoded_size: usize, output: &mut BytesMut) {
    let b64_bytes_written = encode_to_bytes(input, output, config.char_set.encode_table());

    let padding_bytes = if config.pad {
        add_padding_bytes(input.len(), output)
    } else {
        0
    };
...
/// For bufmut
#[inline]
pub fn encode_to_bytes(input: &[u8], output: &mut BytesMut, encode_table: &[u8; 64]) -> usize {
    let mut input_index: usize = 0;
    ...
    if last_fast_index > 0 {
        while input_index <= last_fast_index {
            ...
            output.put_u8(encode_table[((input_u64 >> 58) & LOW_SIX_BITS) as usize]);
            output.put_u8(encode_table[((input_u64 >> 52) & LOW_SIX_BITS) as usize]);
            output.put_u8(encode_table[((input_u64 >> 46) & LOW_SIX_BITS) as usize]);
            output.put_u8(encode_table[((input_u64 >> 40) & LOW_SIX_BITS) as usize]);
            output.put_u8(encode_table[((input_u64 >> 34) & LOW_SIX_BITS) as usize]);
            output.put_u8(encode_table[((input_u64 >> 28) & LOW_SIX_BITS) as usize]);
            output.put_u8(encode_table[((input_u64 >> 22) & LOW_SIX_BITS) as usize]);
            output.put_u8(encode_table[((input_u64 >> 16) & LOW_SIX_BITS) as usize]);
    ...
    while input_index < start_of_rem {
        ...
        output.put_u8(encode_table[(input_chunk[0] >> 2) as usize]);
        output.put_u8(encode_table[((input_chunk[0] << 4 | input_chunk[1] >> 4) & LOW_SIX_BITS_U8) as usize]);
        output.put_u8(encode_table[((input_chunk[1] << 2 | input_chunk[2] >> 6) & LOW_SIX_BITS_U8) as usize]);
        output.put_u8(encode_table[(input_chunk[2] & LOW_SIX_BITS_U8) as usize]);
    ...
    if rem == 2 {
        output.put_u8(encode_table[(input[start_of_rem] >> 2) as usize]);
        output.put_u8(encode_table[((input[start_of_rem] << 4 | input[start_of_rem + 1] >> 4)
            & LOW_SIX_BITS_U8) as usize]);
        output.put_u8(encode_table[((input[start_of_rem + 1] << 2) & LOW_SIX_BITS_U8) as usize]);
        output_index += 3;
    } else if rem == 1 {
        output.put_u8(encode_table[(input[start_of_rem] >> 2) as usize]);
        output.put_u8(encode_table[((input[start_of_rem] << 4) & LOW_SIX_BITS_U8) as usize]);
        output_index += 2;
    }
...
pub fn add_padding_bytes(input_len: usize, output: &mut BytesMut) -> usize {
    let rem = input_len % 3;
    let mut bytes_written = 0;
    for _ in 0..((3 - rem) % 3) {
        output.put_u8(b'=');
